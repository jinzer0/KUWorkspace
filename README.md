...
공유 오피스의 회의실 예약과 장비 대여를 관리하는 CLI 프로그램입니다. 게스트, 일반 사용자, 관리자 흐름이 분리되어 있으며, 모든 데이터는 `data/` 아래의 파이프(`|`) 구분 텍스트 파일에 저장됩니다. 직전 취소, 지연 반납, 이용 제한 같은 운영 규칙도 함께 처리합니다.
...
- 제한 해제, 90일 패널티 초기화, 이용 금지 사용자 미래 예약 자동 취소 같은 정책 자동 처리 제공
...
- 예약 가능 기간은 현재 시점 기준 최대 180일 이내입니다
- 1회 예약 기간은 최대 14일입니다
- 시작 1시간 이내 취소는 `+2점` 패널티가 부여됩니다
- 지연 퇴실 / 반납은 `+2점` 패널티가 부여됩니다
- 패널티가 `3~5점`이면 회의실 1건, 장비 1건까지 각각 유지할 수 있습니다
...
| `src/storage/` | 파이프 구분 텍스트 저장소, 파일 잠금, 원자적 쓰기 구현 |
| `scripts/seed_data.py` | 관리자 계정과 샘플 회의실 / 장비 데이터를 생성하는 스크립트 |
| `data/` | 사용자, 예약, 패널티, 감사 로그 등 런타임 데이터 저장 위치 |
| `tests/` | 단위 / 통합 / E2E 테스트 |
| `docs/PLAN.md` | 구현 범위와 설계 메모 |
| `docs/policy.md`, `docs/opencode_policy.md`, `docs/rule.md` | 현재는 폐기된 보조 문서 |
...
- 회의실: 4인, 6인, 8인 규모 샘플 회의실 각 3개씩(총 9개)
- 장비: 프로젝터, 노트북, 케이블, 웹캠 각각 3개씩(총 12개)
...
- `equipments.txt`
- `room_bookings.txt`
- `equipment_booking.txt`
- `penalties.txt`
- `audit_log.txt`

저장 계층은 `src/storage/repositories.py`에서 파이프 구분 레코드를 읽고 씁니다. 모든 쓰기 경로는 전역 잠금을 요구하며, 여러 파일을 함께 갱신해야 할 때는 `UnitOfWork`로 변경 사항을 스테이징한 뒤 한 번에 커밋합니다.
