...
class TestReservationMemoRules:
    def test_memo_allows_empty_and_exactly_50_chars(self):
        validate_reservation_memo_text("")
        validate_reservation_memo_text("a" * 50)
        assert RESERVATION_MEMO_MAX_LENGTH == 50

    def test_memo_rejects_newline_pipe_and_backslash(self):
        for memo in ("a" * 51, "회의\n준비", "회의\r준비", "회의|준비", r"회의\\준비"):
            with pytest.raises(ValueError):
                validate_reservation_memo_text(memo)


class TestInspect1EquipmentFieldRules:
    def test_inspect1_equipment_direct_input_asset_type_allows_lowercase_ascii_up_to_15_chars(self):
        validate_equipment_asset_type("scanner")
        validate_equipment_asset_type("abcdefghijklmno")

    @pytest.mark.parametrize("asset_type", ["Scanner", "scanner1", "스캐너", "scan ner", "abcdefghijklmnop"])
    def test_inspect1_equipment_direct_input_asset_type_rejects_non_spec_values(self, asset_type):
        with pytest.raises(ValueError):
            validate_equipment_asset_type(asset_type)

    def test_inspect1_equipment_name_requires_hangul_and_keeps_length_and_whitespace_limits(self):
        validate_equipment_name("노트북A1")
        with pytest.raises(ValueError):
            validate_equipment_name("Projector1")
        with pytest.raises(ValueError):
            validate_equipment_name("노트북 A")
        with pytest.raises(ValueError):
            validate_equipment_name("노트북" + "가" * 9)


class TestAuthFieldRules:
    def test_seed_admin_password_remains_valid(self):
    ...
    def test_plan0001_username_password_rules_match_second_plan(self):
        for username in ("Admin1", "User_01"):
            validate_username_text(username)

        for username in ("admin", "abc", "1User", "A user"):
            with pytest.raises(ValueError):
                validate_username_text(username)

        for password in ("abcd1", "A1234"):
            validate_password_text(password)

        for password in ("abcd", "1234", "ab 12"):
            with pytest.raises(ValueError):
                validate_password_text(password)


def test_plan0001_result_screens_require_zero_back(monkeypatch, capsys):
    inputs = iter(["", "0"])
    ...
    assert "계속하려면 Enter" not in output
...
class TestValidatePassword:
    ...
    def test_valid_password_minimum_length(self):
        valid, msg = validate_password("A123")
        assert valid is True
        assert msg == ""

    def test_valid_password_maximum_length(self):
        valid, msg = validate_password("A1" + "a" * 48)
        assert valid is True
        assert msg == ""
    ...
    def test_invalid_password_too_short(self):
        valid, msg = validate_password("A12")
        assert valid is False
        assert "4자 이상" in msg

    def test_invalid_password_too_long(self):
        valid, msg = validate_password("A1" + "a" * 49)
        assert valid is False
    ...
    def test_invalid_password_contains_space(self):
        valid, msg = validate_password("pass word1")
        assert valid is False
        assert "공백" in msg
    ...
    def test_invalid_password_leading_whitespace_stripped(self):
        # Input: "  pass1" contains space characters (leading whitespace),
    ...
    def test_valid_password_case_sensitive(self):
        ...
        valid1, _ = validate_password("Password1")
        valid2, _ = validate_password("password1")
...
class TestValidatorEdgeCases:
    ...
    def test_password_with_all_special_chars_except_space(self):
        ...
        valid, _ = validate_password("p@ss1!#$%^&*()")
        assert valid is True
